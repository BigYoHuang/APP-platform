 ...
 */
const ConstructionApp: React.FC = () => {
  // --- 狀態管理 ---
  ...
  // Changed h-screen to h-full to fit in window container
  if (isLoading) {
    return (
  ...
  if (view === ViewState.HOME) {
    return (
      <div className={`h-full flex flex-col items-center justify-center p-6 animate-fadeIn relative ${appBgClass}`}>
        {/* Changed fixed to absolute to contain blobs */}
        <div className="absolute top-[-10%] left-[-10%] w-[50%] h-[50%] rounded-full bg-cyan-200/30 blur-[80px] pointer-events-none z-0"></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] rounded-full bg-blue-200/30 blur-[100px] pointer-events-none z-0"></div>
        ...
        {showNewProjectDialog && (
          <div className="absolute inset-0 bg-slate-900/20 backdrop-blur-sm flex items-center justify-center z-50 animate-fadeIn">
  ...
  if (view === ViewState.PROJECT_LIST && project) {
    ...
    return (
      <div className={`h-full flex flex-col overflow-hidden relative animate-fadeIn ${appBgClass}`}>
        <div className={`absolute top-[-10%] left-[-10%] w-[50%] h-[50%] rounded-full blur-[80px] pointer-events-none z-0 ${isFirestop ? 'bg-orange-200/30' : 'bg-cyan-200/30'}`}></div>
        <div className={`absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] rounded-full blur-[100px] pointer-events-none z-0 ${isFirestop ? 'bg-red-200/30' : 'bg-blue-200/30'}`}></div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`text-sm px-2 py-0.5 rounded-md border truncate ${
                        isPostConstruction 
                          ...
                          : (isFirestop ? 'bg-orange-50/50 text-orange-700 border-orange-100/50' : 'bg-cyan-50/50 text-cyan-700 border-cyan-100/50') 
                      }`}>
        ...
        {showExitDialog && (
          <div className="absolute inset-0 bg-slate-900/20 backdrop-blur-sm flex items-center justify-center z-50 px-4 animate-fadeIn">
        ...
        {actionDialogRecord && (
           <div 
             className="absolute inset-0 bg-slate-900/20 backdrop-blur-sm flex items-center justify-center z-50 px-4 animate-fadeIn"
             onClick={() => setActionDialogRecord(null)}
           >
             <div 
  ...
  // ViewState.EDIT_RECORD
  if (view === ViewState.EDIT_RECORD && project) {
    return (
      <div className={`h-full w-full relative backdrop-blur-xl animate-fadeIn ${appBgClass}`}>
        <RecordEditor 
          initialData={editingRecord}
          lastRecord={project.records[project.records.length - 1]} 
          defaultValues={recordDefaultValues} 
          projectType={project.type} 
          prefilledDisplayId={nextDisplayId} 
          onSave={saveRecord}
